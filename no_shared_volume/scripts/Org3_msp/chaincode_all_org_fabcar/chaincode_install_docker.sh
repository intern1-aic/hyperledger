...
S=0
	while [[ $S -eq 0 ]]; do
		docker  exec $line /bin/sh -c "peer lifecycle chaincode package fabcar_go-all.tar.gz --path /MSP_Org3/config/chaincode/fabcar/go/ --lang golang --label fabcar_go-all"
