...
c=1
	while [[ $S -eq 0 ]]; do
		...
		kubectl exec $cli -- sh -c "peer lifecycle chaincode commit -o orderer.ai.com:7050 --tls true --cafile /MSP_Org1/config/crypto-config/ordererOrganizations/ai.com/orderers/orderer.ai.com/msp/tlscacerts/tlsca.ai.com-cert.pem --channelID channel1 --name fabcar_1 --peerAddresses peer0.aic.ai.com:7051 --tlsRootCertFiles /MSP_Org1/config/crypto-config/peerOrganizations/aic.ai.com/peers/peer0.aic.ai.com/tls/server.crt --version 1 --sequence 1 --init-required"
		kubectl exec $cli -- sh -c "peer lifecycle chaincode querycommitted --channelID channel1 --name fabcar_1"
