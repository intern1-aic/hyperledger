...
c=1
	while [[ $S -eq 0 ]]; do
		...
		docker  exec $cli /bin/sh -c "peer lifecycle chaincode approveformyorg --tls --cafile /MSP_Org2/config/crypto-config/ordererOrganizations/ai.com/orderers/orderer2.ai.com/msp/tlscacerts/tlsca.ai.com-cert.pem --channelID channel2 --name fabcar_2 --version 1 --init-required --package-id $PACKAGE_ID --sequence 1 --waitForEvent"
		docker  exec $cli /bin/sh -c "peer lifecycle chaincode checkcommitreadiness --channelID channel2 --name fabcar_2 --version 1 --sequence 1 --output json --init-required " >> approve.txt
